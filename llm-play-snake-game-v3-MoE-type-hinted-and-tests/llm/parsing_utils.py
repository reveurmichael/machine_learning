...
def parse_and_format(llm_response: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(f"Error parsing LLM response: {exception}")
        return None
...
def parse_llm_response(
    ...
    try:
        ...
        def _extract_json(text: str):
            """Try extraction helpers in descending order of reliability."""

            # 1) Strict JSON – accepts extra *attempt_id* parameter
            data = extract_valid_json(text, attempt_id=0)
            if data:
                return data

            # 2-4) Helpers that share the *same* ``(str) -> Optional[dict]`` signature
            for helper in (
                extract_json_from_code_block,
                extract_json_from_text,
                extract_moves_from_arrays,
        ...
        if json_data and 'moves' in json_data:
            ...
            if moves and len(moves) > 0:
                print(
                    f"First move: {moves[0]}, Planned moves: {moves[1:] if len(moves) > 1 else []}")
                ...
                filtered_moves = game_instance.filter_invalid_reversals(
                    moves[1:], current_direction)
                if len(filtered_moves) < len(moves[1:]):
                    print(
                        f"Filtered out {len(moves[1:]) - len(filtered_moves)} invalid reversal moves")
        ...
        print(f"Error parsing JSON from LLM response: {exception}")

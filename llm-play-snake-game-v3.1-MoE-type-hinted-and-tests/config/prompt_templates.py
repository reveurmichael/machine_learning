"""Prompt templates for LLM communications in the Snake game."""
...
b. Since new_head == apple position, **grow**: do NOT remove the tail. Now BODY = `[(
    5,
    5),
    (5,
    4),
    (5,
    3
)]` plus the new head will be added next.
4. Update HEAD = (
    6,
    5). The new full S = `[(6,
    5) (head),
...
## OBJECTIVE:
Plan a safe path for the HEAD to reach the APPLE, avoiding collisions and respecting all rules. If you cannot determine a complete safe path to the apple, output as many initial moves as you are confident are safe—even if it's just one. Output a JSON object whose "moves" field is a sequence of directions that takes the head (now at `TEXT_TO_BE_REPLACED_HEAD_POS`) to eat the apple at `TEXT_TO_BE_REPLACED_APPLE_POS`.
...
Now analyze the game state and return only the JSON output as specified.  If you cannot determine a complete safe path to the apple, output as many initial moves as you are confident are safe—even if it's just one. Do **not** include any additional text or code, do not return any python code or other code, do not return Latex—return only the valid JSON object in the exact format.
"""


# Parser prompt for the secondary LLM (formatting expert)
PROMPT_TEMPLATE_TEXT_SECONDARY_LLM = (
    """You are the secondary LLM in a Mixture-of-Experts system for a Snake game. """
    """Your job is to format the primary LLM's output into valid JSON.
...
- "moves" must be a list of directions from: "UP", "DOWN", "LEFT", "RIGHT". 
  Note that directions are uppercase. You might see "U", "D", "L", "R" in the text 
  for representing moves, you should convert them to "UP" (for "U"), "DOWN" (for "D"), 
  "LEFT" (for "L"), "RIGHT" (for "R"); you might see "North", "South", "West", "East" 
  in the text for representing moves, you should convert them to "UP" (for "North"), 
  "DOWN" (for "South"), "LEFT" (for "West"), "RIGHT" (for "East").
- "reasoning" must be a brief explanation for the chosen path
- If after reading RESPONSE_1, according to your understanding, no valid path exists or no safe initial moves can be performed, or if you are not sure about the text's meaning on path and move planning, return your answer in JSON format: { "moves": [], "reasoning": "NO_PATH_OR_SAFE_INITIAL_MOVES_FOUND" } or { "moves": [], "reasoning": "I_CANNOT_UNDERSTAND_THE_TEXT" }

## WHAT YOU SHOULD DO:
- First, if RESPONSE_1 starts with the word "ERROR" (capitalized), 
  you should return { "moves": [], "reasoning": "SOMETHING_IS_WRONG" } in JSON format.
- Second, if RESPONSE_1 contains the <think>...</think> tags, 
  you should ignore THINK_PROCESS_TEXT_OF_RESPONSE_1 and rely solely on 
  FINAL_OUTPUT_TEXT_OF_RESPONSE_1 to figure out your answer in JSON format.
- Third, if FINAL_OUTPUT_TEXT_OF_RESPONSE_1 is not sufficiently informative 
  on moves to perform (e.g. leading to the apple, or one or several initial safe moves), 
  you should use the whole text of RESPONSE_1 to figure out your answer in JSON format.
- Fourth, if RESPONSE_1 does not contain the <think>...</think> tags, 
  you should use the whole text of RESPONSE_1 to figure out your answer in JSON format. 
  Texts at the beginning of RESPONSE_1 are much less important than the texts towards the end.

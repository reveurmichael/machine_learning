...
def render_main_pygame_tab() -> None:
    ...
    with col_core1:
        max_games = st.number_input("Maximum Games", 1, 100,
                                    MAX_GAMES_ALLOWED, 1, key="main_pg_max_games")
    with col_core2:
        max_steps = st.number_input("Max Steps", 10, 1000,
                                    MAX_STEPS_ALLOWED, 10, key="main_pg_max_steps")

    col_time1, col_time2, col_time3 = st.columns(3)
    with col_time1:
        sleep_before = st.number_input("Sleep Before Launch (minutes)",
                                       0.0, 600.0, 0.0, 0.5, key="main_pg_sleep")
    with col_time2:
        move_pause = st.number_input("Move Pause (seconds)", 0.0, 10.0,
                                     PAUSE_BETWEEN_MOVES_SECONDS, 0.1, key="main_pg_move_pause")
    with col_time3:
        sleep_after_empty = st.number_input(
            "Sleep After EMPTY (minutes)", 0.0, 1000.0, SLEEP_AFTER_EMPTY_STEP, 0.5, key="main_pg_sleep_after_empty")
    ...
    if st.button("Start Main Session (PyGame)", key="start_main_pg"):
        ...
        _append_arg(cmd, "--parser-provider", None if parser_provider ==
                    "None" else parser_provider.strip() or None)
        _append_arg(cmd, "--parser-model", None if parser_provider ==
                    "None" else parser_model.strip() or None)
        if max_steps > 0:
...
def render_main_web_tab() -> None:
    ...
    with col2:
        ...
        st.selectbox(
            "Model",
            provider_w_models,
            index=provider_w_models.index(
                DEFAULT_MODEL) if DEFAULT_MODEL in provider_w_models else 0,
            key="main_web_model",
        )
    ...
    with colp2:
        if parser_provider_w == "None":
            ...
            st.selectbox(
                "Parser Model",
                pp_models,
                index=pp_models.index(
                    DEFAULT_PARSER_MODEL) if DEFAULT_PARSER_MODEL in pp_models else 0,
                key="main_web_parser_model",
            )
    ...
    with col_time1:
        sleep_before = st.number_input("Sleep Before Launch (minutes)",
                                       0.0, 600.0, 0.0, 0.5, key="main_web_sleep")
    with col_time2:
        move_pause = st.number_input("Move Pause (seconds)", 0.0, 10.0,
                                     PAUSE_BETWEEN_MOVES_SECONDS, 0.1, key="main_web_move_pause")
    with col_time3:
        sleep_after_empty = st.number_input(
            "Sleep After EMPTY (minutes)", 0.0, 1000.0, SLEEP_AFTER_EMPTY_STEP, 0.5, key="main_web_sleep_after_empty")

    # Safety limits (web)
    col_lim1, col_lim2, col_lim3, col_lim4 = st.columns(4)
    with col_lim1:
        st.number_input("Max Consecutive Empty Moves", 0, 50,
                        MAX_CONSECUTIVE_EMPTY_MOVES_ALLOWED, 1, key="main_web_max_empty_moves")
    with col_lim2:
        st.number_input("Max Consecutive Something-Is-Wrong", 0, 50,
                        MAX_CONSECUTIVE_SOMETHING_IS_WRONG_ALLOWED, 1, key="main_web_max_siw")
    with col_lim3:
        st.number_input("Max Consecutive Invalid Reversals", 0, 50,
                        MAX_CONSECUTIVE_INVALID_REVERSALS_ALLOWED, 1, key="main_web_max_invalid_rev")
    with col_lim4:
        st.number_input("Max Consecutive NO_PATH_FOUND", 0, 50,
                        MAX_CONSECUTIVE_NO_PATH_FOUND_ALLOWED, 1, key="main_web_max_no_path")
    ...
    no_gui = st.checkbox("Run headless (skip GUI & move pauses)",
                         value=False, key="main_web_no_gui")

...
def validate_json_format(data: Any) -> Tuple[bool, Optional[str]]:
    ...
    for i, move in enumerate(data["moves"]):
        ...
        if move_upper not in valid_moves:
            print(
                f"JSON validation error: Invalid move: '{move}' (upper: '{move_upper}'), valid moves are {valid_moves}")
...
def extract_json_from_code_block(response: str) -> Optional[Dict[str, Any]]:
    ...
    for i, code_block in enumerate(code_block_matches):
        try:
            ...
            print(f"❌ JSON decode error in code block {i+1}: {exception}")
            continue
        except Exception as exception:
            print(f"❌ Error during code block extraction for block {i+1}: {exception}")
    ...
    try:
        ...
        print(f"❌ Error during pattern extraction: {exception}")
...
def extract_valid_json(
    ...
    try:
        ...
        # Only print error message on first attempt
        if attempt_id == 0:
            print(f"JSON decode error: {exception}")

    # Try extracting from code block
    json_data: Optional[Dict[str, Any]] = extract_json_from_code_block(text)
    if json_data:
        # Validate the moves format
    ...
    json_data = extract_json_from_text(text)  # type: ignore[assignment]
    if json_data:
        # Validate the moves format
        is_valid, error_msg = validate_json_format(json_data)
        if not is_valid:
    ...
    json_data = extract_moves_from_arrays(text)  # type: ignore[assignment]
    if json_data:
        # Validate the moves format
        is_valid, error_msg = validate_json_format(json_data)
        if not is_valid:
...
def extract_json_from_text(response: str) -> Optional[Dict[str, List[str]]]:
    ...
    moves_dict = extract_moves_pattern(response)
    if moves_dict:
        # extract_moves_pattern already returns the normalized dict structure
        # but we still need to upper-case / validate items for consistency.
        raw_moves: list[str] = moves_dict.get("moves", [])
        standardized_moves = [m.upper() for m in raw_moves]

        valid_moves = [m for m in standardized_moves if m in VALID_MOVES]
        if valid_moves:
...
def extract_moves_pattern(json_str: str) -> Optional[Dict[str, List[str]]]:
    ...
    try:
        # Extract moves array
        moves_array_match = re.search(
            r'["\']?moves["\']?\s*:\s*\[([\s\S]*?)\]', json_str, re.DOTALL)
        ...
        print(f"Move extraction error: {exception}")
        return None

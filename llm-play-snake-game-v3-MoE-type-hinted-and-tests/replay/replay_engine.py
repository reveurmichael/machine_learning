...
from pygame.locals import (  # pylint: disable=no-name-in-module
    ...
    K_r, K_s, K_d, K_n, K_p, K_q, K_f, MOUSEBUTTONDOWN
)
import numpy as np
...
class ReplayEngine(GameController):
    ...
    def load_game_data(self, game_number: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.llm_response = parsed.get(
                'llm_response', "No LLM response data available for this game.")

            print(f"Game {game_number} loaded successfully")
            return game_data

        except Exception as exception:
            print(f"Error loading game data: {exception}")
            traceback.print_exc()
            return None
    ...
    def update(self) -> None:
        ...
        time_elapsed = current_time - self.last_move_time >= self.pause_between_moves
        has_more_moves = self.move_index < len(self.moves)
        if time_elapsed and has_more_moves:
            try:
                # Get next move
                ...
                if self.planned_moves and len(self.planned_moves) > 0:
                    self.planned_moves = self.planned_moves[1:] if len(
                        self.planned_moves) > 1 else []
                ...
                if not game_continues:
                    end_msg = (f"Game {self.game_number} over. Score: {self.score}, "
                              f"Steps: {self.steps}, End reason: {self.game_end_reason}")
                    print(end_msg)

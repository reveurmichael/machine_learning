...
class GameLogic(GameController):
    ...
    def reset(self) -> None:
        ...
        super().reset()
        self.planned_moves.clear()  # Clear existing list instead of reassigning
        self.processed_response = ""
    ...
    def get_state_representation(self) -> str:
        ...
        return prepare_snake_prompt(
            head_position=cast(Sequence[int], self.head_position),
            body_positions=cast(List[Sequence[int]], self.body),
            apple_position=cast(Sequence[int], self.apple_position),

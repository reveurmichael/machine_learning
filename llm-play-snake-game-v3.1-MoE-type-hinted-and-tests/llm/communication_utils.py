...
def check_llm_health(llm_client: LLMClient, max_retries: int = 2, retry_delay: int = 2) -> Tuple[bool, str]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            error_msg = f"Error connecting to {llm_client.provider}/{llm_client.model} LLM: {str(exception)}"
...
def extract_state_for_parser(game_manager: "GameManager") -> Tuple[str, str, str]:
    ...
    """
    # -------------------------------
    # Invariants – a *running* game loop guarantees these attributes but the
    # static type checker cannot infer that.  Add explicit assertions so mypy
    # treats them as non-Optional afterwards.
    # -------------------------------

    from typing import cast

    assert game_manager.game is not None, "GameManager.game must be initialised"
    assert game_manager.prompts_dir is not None, "prompts_dir not set"
    assert game_manager.responses_dir is not None, "responses_dir not set"
    assert game_manager.llm_client is not None, "LLM client not initialised"

    # Get game state - use game_manager.game directly since we've asserted it's not None
...
def get_llm_response(game_manager: "GameManager"):
    ...
    # Start tracking LLM communication time (after ensuring game is not None)
    ...
    prompt_path = save_to_file(prompt, cast(str, game_manager.prompts_dir),
                               prompt_filename, metadata=prompt_metadata)
    ...
    try:
        ...
        if hasattr(game_manager.llm_client, 'last_token_count') and game_manager.llm_client.last_token_count:
            ...
            prompt_tokens_int = int(prompt_tokens) if prompt_tokens is not None else 0
            completion_tokens_int = int(completion_tokens) if completion_tokens is not None else 0
        ...
        response_path = save_to_file(response, cast(
            str, game_manager.responses_dir), response_filename, metadata=response_metadata)
        ...
        if game_manager.args.parser_provider and game_manager.args.parser_provider.lower() != "none":
            ...
            parser_prompt_path = save_to_file(
                parser_prompt, game_manager.prompts_dir, parser_prompt_filename, metadata=parser_prompt_metadata)
            ...
            start_time = time.time()
            secondary_response = game_manager.llm_client.generate_text_with_secondary_llm(
                parser_prompt)
            ...
            if secondary_response is None or "ERROR" in secondary_response:
                print(
                    Fore.YELLOW + "⚠️ Secondary LLM returned an error or no response. Falling back to primary LLM.")
                ...
                if not parser_output:
                    ...
                    code_block_matches = re.findall(
                        r'```(?:json|javascript|js)?\s*([\s\S]*?)```', secondary_response, re.DOTALL)
                    if code_block_matches:
                        for block in code_block_matches:
                            try:
                                ...
                                print(f"Error parsing direct code block: {exception}")
        ...
        if parser_output and "moves" in parser_output and parser_output["moves"]:
            ...
            if next_move:
                ...
                print(
                    Fore.GREEN + f"🐍 Next move: {next_move} (Game {game_manager.game_count+1}, Round {game_manager.round_count})")
        ...
        if game_manager.consecutive_empty_steps >= game_manager.args.max_consecutive_empty_moves_allowed:
            print(
                Fore.RED + f"❌ Maximum consecutive empty moves reached ({game_manager.args.max_consecutive_empty_moves_allowed}). Game over.")
        ...
        print(Fore.RED + f"❌ Error getting response from LLM: {exception}")
        print(Fore.YELLOW +
              f"⚠️ Consecutive LLM errors: {game_manager.consecutive_something_is_wrong}/{game_manager.args.max_consecutive_something_is_wrong_allowed}")
        ...
        if game_manager.consecutive_something_is_wrong >= game_manager.args.max_consecutive_something_is_wrong_allowed:
            print(
                Fore.RED + f"❌ Maximum consecutive LLM errors reached ({game_manager.args.max_consecutive_something_is_wrong_allowed}). Game over.")
            game_manager.game.game_state.record_game_end(

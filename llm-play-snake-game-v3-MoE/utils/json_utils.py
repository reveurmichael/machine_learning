...
def save_session_stats(log_dir, **kwargs):
    ...
    for key, value in kwargs.items():
        if key == "json_error_stats":
            ...
            if value and isinstance(value, dict):
                if "primary" in value:
                    primary_stats = value["primary"]
                    summary["token_usage_stats"]["primary_llm"]["total_tokens"] = primary_stats.get("total_tokens", 0)
                    summary["token_usage_stats"]["primary_llm"]["total_prompt_tokens"] = primary_stats.get("total_prompt_tokens", 0)
                    summary["token_usage_stats"]["primary_llm"]["total_completion_tokens"] = primary_stats.get("total_completion_tokens", 0)
                if "secondary" in value:
                    secondary_stats = value["secondary"]
                    summary["token_usage_stats"]["secondary_llm"]["total_tokens"] = secondary_stats.get("total_tokens", 0)
                    summary["token_usage_stats"]["secondary_llm"]["total_prompt_tokens"] = secondary_stats.get("total_prompt_tokens", 0)
                    summary["token_usage_stats"]["secondary_llm"]["total_completion_tokens"] = secondary_stats.get("total_completion_tokens", 0)
        elif key == "step_stats":

...
Let's put everything together into a step-by-step tutorial that you can follow during the 90-minute session:

```python
def run_mnist_tutorial():
    """Complete walkthrough of neural network implementation on MNIST"""
    print("\n========== STEP 1: UNDERSTANDING NEURAL NETWORK ARCHITECTURE ==========")
    # Visualize a simple neural network architecture
    plot_neural_network(num_input=5, num_hidden=7, num_output=3)
    ...
    print("\n========== STEP 4: LOADING AND EXPLORING THE MNIST DATASET ==========")
    # Load the MNIST dataset
    X_train, y_train, X_val, y_val, X_test, y_test = load_mnist_from_csv('mnist_train.csv', 'mnist_test.csv')
    
    # Visualize sample images
    visualize_mnist_samples(X_train, y_train, num_samples=8)
    input("Press Enter to continue...")
    
    print("\n========== STEP 5: TESTING THE RELU ACTIVATION LAYER ==========")
    # Test ReLU implementation
    test_relu_layer()
    input("Press Enter to continue...")
    
    print("\n========== STEP 6: TESTING THE DENSE LAYER ==========")
    # Test Dense layer implementation
    test_dense_layer()
    input("Press Enter to continue...")
    
    print("\n========== STEP 7: TRAINING A SMALL NETWORK ON MNIST ==========")
    # For the demo, use a very small network and few epochs
    network = [
        Dense(784, 32),  # Input -> Hidden (32 neurons)
        ReLU(),          # Activation
        Dense(32, 10)    # Hidden -> Output (10 classes)
    ]
    
    # Use a small subset for quick demonstration
    subset_size = 5000  # Adjust based on available time
    subset_indices = np.random.choice(len(X_train), subset_size, replace=False)
    ...
    # Mini-batch training
    batch_size = 32
    num_epochs = 2  # Just 2 epochs for the demo
    
    print(f"Training a small network on {subset_size} examples for {num_epochs} epochs")
...
# Uncomment this line to run the complete tutorial
# trained_network = run_mnist_tutorial()
```

...
class SnakeGame:
    ...
    def make_move(self, direction):
        ...
        # 检查是否撞到自己
        if any(np.array_equal(head, pos) for pos in self.snake_positions[1:]):
            return False, False
        
        # 将新头部添加到蛇身数组的开头
        self.snake_positions = np.vstack([head, self.snake_positions])
        
        # 检查是否吃到苹果
        apple_eaten = False
        if np.array_equal(head, self.apple_position):
            apple_eaten = True
            self.score += 1
            # 生成新的苹果位置
            self.generate_apple()
    ...
    def generate_apple(self):
            ...
            while True:
                ...
                # 确保苹果不会生成在蛇身上
                if not any(np.array_equal(new_position, pos) for pos in self.snake_positions):

"""
A* Game Manager - Session management for A* pathfinding algorithm
================================================================

This module extends BaseGameManager to provide session management
specifically for A* pathfinding algorithm while maintaining compatibility
...
Key Features:
- More efficient pathfinding than BFS
...
class HeuristicGameManager(BaseGameManager):
    """
    Session manager for A* pathfinding algorithm.
    
    Extends BaseGameManager with A*-specific functionality while
    ...
    - Strategy Pattern: A* pathfinding algorithm
    """
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.algorithm_name: str = getattr(args, 'algorithm', 'A*')
        self.agent: Optional[AStarAgent] = None
    ...
    def initialize(self) -> None:
        ...
        print(Fore.GREEN + f"🤖 A* Game Manager initialized with {self.algorithm_name} algorithm")
    ...
    def _setup_logging(self) -> None:
        ...
        self.log_dir = f"logs/heuristics-v0.02-astar_{timestamp}"
    ...
    def _setup_agent(self) -> None:
        """Setup the A* agent."""
        if self.algorithm_name.upper() in ["A*", "ASTAR", "A_STAR"]:
            self.agent = AStarAgent()
        else:
            # Default to A* for now
            print(Fore.YELLOW + f"⚠️  Algorithm '{self.algorithm_name}' not implemented, using A*")
            self.agent = AStarAgent()

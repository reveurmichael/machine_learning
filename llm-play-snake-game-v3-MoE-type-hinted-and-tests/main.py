...
def main():
    ...
    try:
        ...
        try:
            ...
            print(Fore.RED + f"Command-line error: {exception}")
        ...
        if args.continue_with_game_in_dir:
            ...
            print(Fore.GREEN +
                  f"🔄 Continuing from existing session: {args.continue_with_game_in_dir}")
        ...
        if args.parser_provider and args.parser_provider.lower() != 'none':
            ...
            if not secondary_env_ok:
                print(
                    Fore.YELLOW + f"⚠️ Warning: Secondary LLM ({args.parser_provider}) environment setup issues detected")

        if not primary_env_ok:
            user_choice = input(
                Fore.YELLOW + "Environment setup issues detected. Continue anyway? (y/n): ")
        ...
        print(Fore.RED + f"Fatal error: {exception}")
        import traceback
        traceback.print_exc()
    finally:
        # Clean up pygame

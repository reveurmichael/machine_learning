    ...
    def _get_best_ollama_model(self, server: str = None) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not models:
                    print(
                        f"No Ollama models found on server {server}. Using fallback model: {fallback_model}"
                    )
                ...
                for model in models:
                    ...
                    size_mb = (
                        model.get("size") / (1024 * 1024) if model.get("size") else 0
                    )
                ...
                if models_with_size:
                    ...
                    print(
                        f"Selected largest available model on server {server}: {best_model}"
                    )
                    return best_model
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    if models:
                        ...
                        best_model = models[0]
                        print(
                            f"Selected model with largest parameter count: {best_model}"
                        )
                        return best_model
            except Exception as e:

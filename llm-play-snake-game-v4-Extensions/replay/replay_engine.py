...
class BaseReplayEngine(BaseGameController):
    ...
    def __init__(
        ...
        # Timing helper used by update(); initialise to *now* so the first
        # move obeys the configured pause interval.
    ...
    def load_game_data(self, game_number: int):  # pragma: no cover
        """Load *game_number* into memory (subclasses must implement)."""
        ...
        if (
            ...
            try:
                ...
                if not game_continues:
                    ...
                    # Advance to next game if auto_advance is enabled
...
class ReplayEngine(BaseReplayEngine, GameController):
    ...
    def update(self) -> None:  # Overrides abstract base
        """Advance one frame in the replay timeline."""

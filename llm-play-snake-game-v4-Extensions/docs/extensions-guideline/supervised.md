...
> **Important — Authoritative Reference:** This document serves as a **GOOD_RULES** authoritative reference for supervised learning extension standards and supplements the _Final Decision Series_ and extension guidelines.
...
- **SUPREME_RULE NO.1**: Enforces reading all GOOD_RULES before making supervised learning architectural changes to ensure comprehensive understanding
- **SUPREME_RULE NO.2**: Uses precise `final-decision-N.md` format consistently when referencing architectural decisions and ML patterns
- **SUPREME_RULE NO.3**: Enables lightweight common utilities with OOP extensibility while maintaining supervised learning patterns through inheritance rather than tight coupling

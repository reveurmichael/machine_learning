...
This module implements a token-limited BFS agent (4096 tokens) that inherits
from the standard BFS agent but generates full detailed explanations.
...
class BFS4096TokenAgent(BFSAgent):
    """
    BFS Agent with full 4096-token explanations.
    ...
    def _generate_basic_explanation(self, game_state: dict, path: List[Tuple[int, int]], 
        ...
        """
        Generate full detailed explanation for the chosen move (4096 tokens max).

...
from .agent import DQNAgent  # 或 DQNAgent
...
def test_model(model_path='snakeq_rl/model/dqn/dqn_best.pth'):  # 或 dqn_best.pth
    # 初始化环境和智能体
    env = SnakeGame()
    state_dim = 28
    action_dim = 4
    ...
    # 创建智能体并加载模型
    agent = DQNAgent(state_dim, action_dim, device)  # 或 DQNAgent
    agent.load(model_path)
    
    # 初始化pygame
    pygame.init()
    pygame.font.init()
    pygame.display.set_caption("DQN Snake - Testing")  # 或 "DQN Snake - Testing"

...
OUTPUT_FOLDER_CN_WITH_FONT_SIZE = "./output_llm_cn_with_font_size"  # Chinese-only with font size
OUTPUT_FOLDER_EN_WITH_FONT_SIZE = "./output_llm_en_with_font_size"  # English-only with font size
...
def extract_first_json_object(text):
    ...
    text = re.sub(r'^.*?(\[|\{)', r'\1', text)  # Remove any text before the first '[' or '{'
    text = re.sub(r'(\]|\})[^[\]{}]*$', r'\1', text)  # Remove any text after the last ']' or '}'
    ...
    if "'" in text:
        ...
        text = re.sub(r'\'([^\']*?)\'\s*,', r'"\1",', text)     # Array elements followed by comma
    ...
    text = re.sub(r':\s*"(True|False|None)"', lambda m: 
            ...
            if potential_json:
                ...
                try:
                    ...
                    if "Expecting property name" in error_msg and position >= 0:
                        ...
                        potential_json = re.sub(r'([}\]])(\s*)([{["a-zA-Z0-9])', r'\1,\2\3', potential_json)
                    ...
                    try:
                        ...
                        try:
                            ...
                            objects = re.findall(r'{[^{}]*(?:{[^{}]*}[^{}]*)*}', potential_json)
                            if objects:
                    ...
                    if position >= 0:
                        if "Expecting property name" in error_msg:
                            ...
                            if re.search(r'"[^"]+"\s*(?![,:}])', prefix):
                                ...
                                new_prefix = prefix[:last_quote+1] + ":" + prefix[last_quote+1:]
        ...
        if objects:
            ...
            for obj in objects:
                try:
                    ...
                    fixed_obj = re.sub(r'([{,])\s*([a-zA-Z_]\w*)(\s*:)', r'\1"\2"\3', obj)
                    fixed_obj = re.sub(r':\s*"(true|false|null)"', r': \1', fixed_obj, flags=re.IGNORECASE)
    ...
    for pattern in fallback_patterns:
        try:
            ...
            if match:
                ...
                potential_json = re.sub(r'([{,])\s*([a-zA-Z_]\w*)(\s*:)', r'\1"\2"\3', potential_json)
                potential_json = re.sub(r':\s*"(true|false|null)"', r': \1', potential_json, flags=re.IGNORECASE)
    ...
    for block in content.split('\n\n'):
        ...
        if len(lines) >= 3:
            try:
                ...
                if start_ms >= end_ms:
                    tqdm_warning(f"Warning: Entry {orig_index} has invalid timing (start >= end), fixing")
...
def reindex_entries(srt_content):
    ...
    if skipped_count > 0:
        tqdm_warning(f"Skipped {skipped_count} malformed or empty entries during reindexing")
    ...
    try:
        ...
        tqdm_warning(f"Warning: Error sorting entries by timestamp: {str(e)}. Using original order.")
...
def get_translation_for_batch(batch_entries):
    ...
    try:
        ...
        response_content = re.sub(r'```(?:json|javascript|js)?\s*', '', response_content)
...
def validate_srt_file(file_path):
    ...
    try:
        ...
        if not re.search(r'\d+\s+\d{2}:\d{2}:\d{2}[,\.]\d{3}\s+-->\s+\d{2}:\d{2}:\d{2}[,\.]\d{3}', content):
            return False, f"File {file_name} does not appear to be a valid SRT file (missing timestamp format)"
        ...
        try:
            with open(file_path, 'r', encoding='latin-1') as f:
                content = f.read()
            return False, f"File {file_name} has encoding issues (not UTF-8). Consider converting to UTF-8."
        except:
    ...
    while i < len(subtitle_entries):
        ...
        while j < len(subtitle_entries):
            ...
            if current_text_lower in next_text_lower:
                ...
                current_end_ms = timestamp_to_milliseconds(subtitle_entries[contained_entries[-1]].end_time)
                next_start_ms = timestamp_to_milliseconds(subtitle_entries[j].start_time)
...
def create_subtitle_entries(index, start_time, end_time, english_text, chinese_text):
    ...
    bilingual_entry_with_font = SubtitleEntry(index, start_time, end_time, bilingual_text_with_font)
    ...
    return (bilingual_entry, bilingual_entry_with_font, eng_entry, chi_entry, eng_entry_with_font, chi_entry_with_font)
...
def add_entries_to_content_lists(entries, content_lists):
    ...
    """
    bilingual_entry, bilingual_entry_with_font, eng_entry, chi_entry, eng_entry_with_font, chi_entry_with_font = entries
    translated_srt_content, translated_srt_content_with_font, english_srt_content, chinese_srt_content, english_srt_content_with_font, chinese_srt_content_with_font = content_lists
...
def process_entry_group(valid_entries, improved_english, chinese_translation, content_lists):
...
def process_batch_entries(processed_entries, filtered_batch, processed_indices, unprocessed_indices, content_lists):
    ...
    for indices, entries in entries_by_indices.items():
        ...
        for proc_entry in entries:
            ...
            for entry in valid_entries:
                ...
                if unprocessed_indices is not None and entry.index in unprocessed_indices:
            ...
            success = process_entry_group(valid_entries, improved_english, chinese_translation, content_lists)
            ...
            if not success:
                ...
                for entry in valid_entries:
                    ...
                    if unprocessed_indices is not None and entry.index not in unprocessed_indices:
...
def process_untranslated_entries(subtitle_entries, processed_indices, content_lists, file_name):
    ...
    for entry in subtitle_entries:
        if entry.index in unprocessed_indices:
            ...
            tqdm_warning(f"Using original text for entry {entry.index} in {file_name} (translation failed)")
            untranslated_count = create_untranslated_entry(entry, untranslated_count, content_lists)
    
    if untranslated_count > 0:
        tqdm_warning(f"Added {untranslated_count} untranslated entries with original text")
...
def process_file(file_path, output_path):
    ...
    process_retry_batches(subtitle_entries, processed_indices, content_lists, max_retries=None)
    
    # Process any entries that weren't translated even after retries
    process_untranslated_entries(subtitle_entries, processed_indices, content_lists, file_name)
    ...
    for entry_text in srt_content:
        ...
        if len(lines) >= 2:
            try:
                ...
                text = '\n'.join(lines[2:])
                start_time, end_time = time_line.split(' --> ')
                entries.append(SubtitleEntry(index, start_time.strip(), end_time.strip(), text.strip()))
...
def fix_overlapping_subtitles(srt_content):
    ...
    if short_duration_count > 0 or fixed_count > 0:
        return [format_srt_entry(entry) for entry in entries], short_duration_count, fixed_count
    else:
        return srt_content, 0, 0
...
def reindex_and_fix_all_outputs(content_lists, file_name):
    ...
    if translated_srt_content:
        tqdm_success(f"Successfully created {len(translated_srt_content)} subtitle entries for {file_name}")
        ...
        if sorted(all_indices) != list(range(1, len(all_indices)+1)):
            tqdm.write(f"CRITICAL: Non-consecutive indices after STATE_NEW transformation in {file_name}")
...
def write_output_files(content_lists, output_paths):
    ...
    """
    translated_srt_content, translated_srt_content_with_font, english_srt_content, chinese_srt_content, english_srt_content_with_font, chinese_srt_content_with_font = content_lists
    output_path, output_path_with_font, output_path_en, output_path_cn, output_path_en_with_font, output_path_cn_with_font = output_paths

    # Add the AI translated marker to each file
    final_content = "".join(translated_srt_content) + f"{AI_TRANSLATED_MARKER}\n"
    final_content_with_font = "".join(translated_srt_content_with_font) + f"{AI_TRANSLATED_MARKER}\n"
    ...
    final_content_en_with_font = "".join(english_srt_content_with_font) + f"{AI_TRANSLATED_MARKER}\n"
    final_content_cn_with_font = "".join(chinese_srt_content_with_font) + f"{AI_TRANSLATED_MARKER}\n"
...
def print_config(args):
    ...
    print_info(f"Bilingual with font size output folder: {OUTPUT_FOLDER_WITH_FONT_SIZE}")
    print_info(f"Chinese-only with font size output folder: {OUTPUT_FOLDER_CN_WITH_FONT_SIZE}")
    print_info(f"English-only with font size output folder: {OUTPUT_FOLDER_EN_WITH_FONT_SIZE}")
...
def process_retry_batches(subtitle_entries, processed_indices, content_lists, max_retries=None):
    ...
    tqdm_info(f"Retrying {len(unprocessed_indices)} failed entries with {max_retries} retry rounds...")
    ...
    for round_num in range(max_retries):
        ...
        if round_num > 0:
            ...
            tqdm_info(f"Retry round {round_num+1}/{max_retries}: Waiting {jittered_delay:.1f}s before next round...")
            time.sleep(jittered_delay)
            retry_delay = min(16, 2 * retry_delay)  # Double delay for next round, cap at 16 seconds    
        else:
        ...
        for batch in retry_batches:
            should_skip, filtered_batch = filter_batch_by_processed(batch, processed_indices)
            if should_skip:
                continue
            ...
            try:
                ...
                process_batch_entries(processed_entries, filtered_batch, processed_indices, unprocessed_indices, content_lists)
            except Exception as e:
                entry_indices = [e.index for e in filtered_batch]
                tqdm_error(f"Error processing batch with indices {entry_indices}: {str(e)}")
            ...
            if batch_size > 1 and round_num == max_retries - 1:
                ...
                still_unprocessed = [e for e in filtered_batch if e.index in unprocessed_indices]
                if still_unprocessed:
                    tqdm_info(f"Last round: Attempting individual processing for {len(still_unprocessed)} entries...")
                    for entry in still_unprocessed:
                        try:
                            single_batch = [entry]
                            processed_entries = get_translation_for_batch(single_batch)
                            process_batch_entries(processed_entries, single_batch, processed_indices, unprocessed_indices, content_lists)
                        except Exception:
                            pass
        ...
        if not retry_entries:
            ...
            tqdm_info(f"Round {round_num+1} completed: {initial_unprocessed_count - remaining} recovered, {remaining} still pending")
    ...
    if successfully_retried > 0:
        tqdm_success(f"Retry successful: Recovered {successfully_retried} of {initial_unprocessed_count} failed entries")

    if unprocessed_indices:
        tqdm_warning(f"Retry failed: {len(unprocessed_indices)} entries could not be processed after {max_retries} attempts")
...
def process_batches(subtitle_entries, processed_indices, content_lists, file_name):
    ...
    # Process entries in batches
    for batch in tqdm(list(create_entry_batch(subtitle_entries)), desc=f"Processing {file_name}", leave=False):
        should_skip, filtered_batch = filter_batch_by_processed(batch, processed_indices)
        if should_skip:
            continue
        ...
        try:
            ...
            count = process_batch_entries(processed_entries, filtered_batch, processed_indices, None, content_lists)
            success_count += count
...
def preprocess_subtitle_entries(subtitle_entries, file_name):
    ...
    if processed_count < original_count:
        tqdm_info(f"Processed {original_count} entries into {processed_count} STATE_NEW entries")
...
def apply_to_all_content_lists(content_lists, operation, *args, **kwargs):
    ...
    translated_srt_content_with_font = operation(translated_srt_content_with_font, *args, **kwargs) 
    ...
    english_srt_content_with_font = operation(english_srt_content_with_font, *args, **kwargs)
    chinese_srt_content_with_font = operation(chinese_srt_content_with_font, *args, **kwargs)
...
def get_unprocessed_indices(subtitle_entries, processed_indices):
    ...
    """
    return {entry.index for entry in subtitle_entries if entry.index not in processed_indices}
...
def main():
    ...
    try:
        ...
        parser.add_argument('--no-random-order', '-n', action='store_true', 

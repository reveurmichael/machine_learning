...
def _process_active_game(game_manager: "GameManager") -> None:
    ...
    # Ensure game is initialized (should be guaranteed by caller, but help mypy)
...
def _post_apple_logic(game_manager: "GameManager") -> None:
    ...
    if game_manager.game.planned_moves:
        print(
            Fore.CYAN + f"Continuing with {len(game_manager.game.planned_moves)} remaining planned moves for this round.")
    else:
        game_manager.finish_round()
...
def _handle_no_move(game_manager: "GameManager") -> None:
    ...
    max_empty_allowed = game_manager.args.max_consecutive_empty_moves_allowed
    print(Fore.YELLOW +
          f"⚠️ No valid moves found. Empty moves: {game_manager.consecutive_empty_steps}/{max_empty_allowed}")
    ...
    if game_manager.consecutive_empty_steps >= max_empty_allowed:
        print(
            Fore.RED + f"❌ Maximum consecutive empty moves reached ({max_empty_allowed}). Game over.")
...
def _execute_move(manager: "GameManager", direction: str) -> Tuple[bool, bool]:
    ...
    # Ensure game is initialized
    assert manager.game is not None, "GameManager.game must be initialized"

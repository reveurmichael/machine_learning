...
class GameController:
    ...
    def __init__(self, grid_size: int = GRID_SIZE, use_gui: bool = True) -> None:
        ...
        self.head_position: NDArray[np.int_] = self.snake_positions[-1]
        ...
        # Runtime trackers
        self.current_direction: Optional[NDArray[np.int_]] = None
        self.last_collision_type: Optional[str] = None
        ...
        self.gui: Optional["BaseGUI"] = None
    ...
    def _update_board(self) -> None:
        ...
        for pos_x, pos_y in self.snake_positions:
            self.board[pos_y, pos_x] = self.board_info["snake"]

        # Place the apple
        apple_x, apple_y = self.apple_position
        self.board[apple_y, apple_x] = self.board_info["apple"]
    ...
    def _generate_apple(self) -> NDArray[np.int_]:
        ...
        while True:
            ...
            rand_x, rand_y = np.random.randint(0, self.grid_size, 2)

            # Check if position is empty (not occupied by snake)
            if not any(np.array_equal([rand_x, rand_y], pos) for pos in self.snake_positions):
    ...
    def set_apple_position(self, position: List[int]) -> bool:
        ...
        """
        try:
            apple_x, apple_y = position

            # Validate position
            if apple_x < 0 or apple_x >= self.grid_size or apple_y < 0 or apple_y >= self.grid_size:
                print(f"Invalid apple position: {position}")
                return False

            # Check if position is empty
            if any(np.array_equal([apple_x, apple_y], pos) for pos in self.snake_positions):
                print(f"Cannot place apple on snake: {position}")
                return False

            # Set the position
            self.apple_position = np.array([apple_x, apple_y])
            ...
            print(f"Error setting apple position: {exception}")
            return False
    ...
    def make_move(self, direction_key: str) -> Tuple[bool, bool]:
        ...
        # Update current direction (convert tuple to NDArray)
        self.current_direction = np.array(direction)
        ...
        # Debug log
        print(
            f"Moving {direction_key}: Head from ({head_x}, {head_y}) to ({new_head[0]}, {new_head[1]})")
        ...
        wall_collision, body_collision = self._check_collision(
            new_head, is_eating_apple_flag=is_eating_apple_at_new_head)

...
"""
BFS Safe Greedy 4096 Token Agent - Full detailed SAFE-GREEDY pathfinding for Snake Game v0.04
----------------
...
class BFSSafeGreedy4096TokenAgent(BFSSafeGreedyAgent):
    ...
    Inheritance Pattern:
    - Inherits from BFSSafeGreedyAgent (reuses all SAFE-GREEDY logic)
    ...
    def __init__(self):
        """Initialize BFS Safe Greedy 4096-token agent, extending base SAFE-GREEDY."""
        super().__init__()  # Initialize parent SAFE-GREEDY agent
    ...
    def _generate_move_explanation(self, game_state: dict, path: List[Tuple[int, int]], 
        ...
        """
        Generate full detailed SAFE-GREEDY explanation for the chosen move (4096 tokens max).
        ...
        # Full detailed SAFE-GREEDY explanation (4096 tokens)
        explanation_parts = [
            ...
            "",
            "PHASE 3: SAFE-GREEDY STRATEGY EXECUTION",
            f"• Algorithm: BFS-Safe-Greedy with safety validation",
            f"• Primary strategy: Find shortest path to apple using BFS",
            f"• Secondary strategy: Validate path safety (tail reachability)",
            f"• Tertiary strategy: Tail-chasing if apple path unsafe",
            ...
            "• Safety check: Can snake reach tail after this move?",
            "• Validation method: BFS pathfinding from new head to tail",
            "• Safety result: VALIDATED SAFE (tail reachable after move)",
            "• Risk assessment: LOW (safe move with guaranteed escape route)",
            "",
            "PHASE 6: MOVE SELECTION LOGIC",
            f"• Chosen direction: {direction}",
            f"• Next position: {next_pos}",
            "• Rationale: First step of shortest path to apple (SAFE VALIDATED)",
            "• Strategy phase: SAFE_APPLE_PATH",
            "",
            ...
            "• Safety guarantee: Tail reachability ensures escape route",
            "• Risk mitigation: SAFE-GREEDY guarantees both progress and safety",
            "",
            "=== DECISION SUMMARY ===",
            f"Moving {direction} follows the BFS-Safe-Greedy computed {'optimal' if is_optimal else 'best available'} path " +
            ...
            f"Safety validation confirms tail reachability, ensuring both progress and escape capability. " +

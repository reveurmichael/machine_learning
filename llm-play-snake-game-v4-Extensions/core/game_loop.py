...
from utils.game_manager_utils import check_max_steps, process_game_over
...
def run_game_loop(game_manager: "GameManager") -> None:
    ...
    try:
        while game_manager.running and game_manager.game_count < game_manager.args.max_games:
            # Advance the actual game logic.
            if game_manager.game_active and game_manager.game is not None:
                if getattr(game_manager, "agent", None) is not None:
                    _process_agent_game(game_manager)
                else:
...
def _request_and_execute_first_move(game_manager: "GameManager") -> None:
    ...
    # So don't change this functionality.
    game_manager.game.draw()
    if game_manager.use_gui:
        time.sleep(3)
    ...
    _, apple_eaten = _execute_move(game_manager, next_move)
...
# ---------------------------
# Alternative gameplay path – *external* agent implementing SnakeAgent
# ---------------------------


def _process_agent_game(game_manager: "GameManager") -> None:  # noqa: C901 – complexity mirrors core path
    """Single-tick handler when a pluggable `SnakeAgent` drives the snake.

    The logic is deliberately **minimal** so we do not disturb the rich
    bookkeeping implemented elsewhere for the LLM path.  Crucially, we rely
    on the existing `_execute_move`, `_handle_no_move`, `_handle_game_over`
    helpers so statistics, invalid-reversal tracking, and GUI refreshes behave
    identically.
    """

    # Ask the agent for a move.  A return value of ``None`` means "no valid
    # idea" which we treat exactly like the EMPTY sentinel used in the LLM
    # pipeline so downstream analytics remain consistent.
    move = game_manager.agent.get_move(game_manager.game)  # type: ignore[arg-type]

    if not move:
        _handle_no_move(game_manager)
    else:
        game_manager.current_game_moves.append(move)
        game_manager.game.draw()
        _, apple_eaten = _execute_move(game_manager, move)

        # Basic apple-eaten branch – we *do not* rely on planned moves when an
        # external agent is in control, so we skip the round / plan logic.
        if apple_eaten:
            print(Fore.CYAN + "🍎 Apple eaten!")

    # ---------------- Post-turn end-of-game handling -------------------
    if not game_manager.game_active:
        _handle_game_over(game_manager)

    # Always refresh the GUI (mirrors LLM path)
    game_manager.game.draw()

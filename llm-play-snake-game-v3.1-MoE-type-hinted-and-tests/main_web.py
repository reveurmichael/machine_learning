...
def build_state_dict(gm: GameManager):
    ...
    snake = game.snake_positions.tolist() if hasattr(
        game.snake_positions, 'tolist') else game.snake_positions
    apple = game.apple_position.tolist() if hasattr(
        game.apple_position, 'tolist') else game.apple_position
    ...
    try:
        ...
        if cont_dir:
            try:
                ...
                print(f"[main_web] Continuation session crashed: {exception}")
                return
        else:
            gm.run()
    except Exception as exception:
        print(f"[main_web] GameManager thread crashed: {exception}")
...
def api_control():
    ...
    cmd = request.json.get('command') if request.is_json and request.json is not None else None
    if cmd == 'pause':
...
def main():
    ...
    manager_thread = threading.Thread(target=manager_thread_fn,
                                      args=(manager, game_args), daemon=True)

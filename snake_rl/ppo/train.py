...
def main():
    # 创建模型保存目录
    os.makedirs('snakeq_rl/model/ppo', exist_ok=True)
    ...
    print(f"PPO更新间隔: {update_interval}")
    print("=" * 50)
    ...
    for episode in range(max_episodes):
        ...
        if (episode + 1) % 100 == 0:
            ...
            if avg_reward > best_reward:
                ...
                agent.save('snakeq_rl/model/ppo/ppo_best.pth')
                print(f"新的最佳模型已保存! 平均奖励: {best_reward:.2f}")
        
        # 定期保存模型
        if (episode + 1) % 1000 == 0:
            agent.save(f'snakeq_rl/model/ppo/ppo_episode_{episode + 1}.pth')
            print(f"模型已保存: ppo_episode_{episode + 1}.pth")

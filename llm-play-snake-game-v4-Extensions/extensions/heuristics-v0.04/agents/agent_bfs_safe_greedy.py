...
class BFSSafeGreedyAgent(BFSAgent):
    ...
    def get_move_with_explanation(self, state: dict) -> Tuple[str, dict]:
        ...
        if path_to_apple and len(path_to_apple) > 1:
            ...
            if self._is_move_safe(state, next_pos):
                ...
                explanation_dict = self._generate_basic_safe_apple_explanation(state, path_to_apple, direction, valid_moves, 
        ...
        if path_to_tail and len(path_to_tail) > 1:
            ...
            explanation_dict = self._generate_basic_tail_chase_explanation(state, tail, path_to_tail, direction, valid_moves, manhattan_distance, remaining_free_cells, metrics)
        ...
        if valid_moves:
            ...
            explanation_dict = self._generate_basic_survival_explanation(state, direction, valid_moves, 
                                                                 manhattan_distance, remaining_free_cells, metrics)
            ...
            explanation_dict = self._generate_basic_no_moves_explanation(state, valid_moves, 
                                                                 manhattan_distance, remaining_free_cells, metrics)
            return direction, explanation_dict
    ...
    def _is_move_safe(self, game_state: dict, next_pos: List[int]) -> bool:
        ...
        new_snake = [list(next_pos)] + snake_positions[:-1]  # Head moves, tail disappears
        new_obstacles = set(tuple(p) for p in new_snake[1:])  # All body segments except new head

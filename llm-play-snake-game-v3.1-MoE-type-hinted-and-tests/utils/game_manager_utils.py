...
def process_game_over(game: "GameLogic", game_state_info: Dict[str, Any]):
    ...
    if hasattr(game.game_state, "get_token_stats"):
        ...
        if primary_total is not None:
            token_stats["primary"]["total_tokens"] = token_stats["primary"].get(
                "total_tokens", 0) + primary_total
        if primary_prompt is not None:
            token_stats["primary"]["total_prompt_tokens"] = token_stats["primary"].get(
                "total_prompt_tokens", 0) + primary_prompt
        if primary_completion is not None:
            token_stats["primary"]["total_completion_tokens"] = token_stats["primary"].get(
                "total_completion_tokens", 0) + primary_completion
        ...
        if secondary_total is not None:
            token_stats["secondary"]["total_tokens"] = token_stats["secondary"].get(
                "total_tokens", 0) + secondary_total
        if secondary_prompt is not None:
            token_stats["secondary"]["total_prompt_tokens"] = token_stats["secondary"].get(
                "total_prompt_tokens", 0) + secondary_prompt
        if secondary_completion is not None:
            token_stats["secondary"]["total_completion_tokens"] = token_stats["secondary"].get(
                "total_completion_tokens", 0) + secondary_completion
...
def initialize_game_manager(game_manager: "GameManager") -> None:
    ...
    model_info_path = save_experiment_info_json(game_manager.args, cast(str, game_manager.log_dir))
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            if event.key == K_ESCAPE:
                ...
                # Reset game
                assert game_manager.game is not None, "GameManager.game must be initialized"

...
def setup_continuation_session(
    ...
    """Load data from an existing session to continue gameplay.

    Sets up the game manager's state including directories, score counters,
    and time/token stats to continue from where the previous session left off.

    Args:
        game_manager: The GameManager instance to configure
        log_dir: Directory containing the session logs
    ...
    prompts_dir: str = os.path.join(log_dir, "prompts")
    responses_dir: str = os.path.join(log_dir, "responses")
    ...
    time_stats: defaultdict[str, int] = defaultdict(int)
    time_stats.update({
    ...
    game_manager.time_stats = time_stats
    ...
    token_stats: dict[str, defaultdict[str, int]] = {
        "primary": defaultdict(int),
        "secondary": defaultdict(int),
    }
    token_stats["primary"].update({
    ...
    game_manager.token_stats = token_stats
...
def handle_continuation_game_state(game_manager: "GameManager") -> None:
    ...
    # Ensure game.game_state is not None before accessing attributes
    if game_manager.game and game_manager.game.game_state:
...
def continue_from_directory(
    ...
    try:
        ...
        if 'configuration' in summary_data:
            ...
            args.max_consecutive_empty_moves_allowed = original_config.get(
                'max_consecutive_empty_moves_allowed', args.max_consecutive_empty_moves_allowed)
            args.max_consecutive_something_is_wrong_allowed = original_config.get(
                'max_consecutive_something_is_wrong_allowed', args.max_consecutive_something_is_wrong_allowed)
            args.max_consecutive_invalid_reversals_allowed = original_config.get(
                'max_consecutive_invalid_reversals_allowed', args.max_consecutive_invalid_reversals_allowed)
            args.max_consecutive_no_path_found_allowed = original_config.get(
                'max_consecutive_no_path_found_allowed', args.max_consecutive_no_path_found_allowed)
            args.sleep_after_empty_step = original_config.get(
                'sleep_after_empty_step', args.sleep_after_empty_step)
            ...
            print(Fore.GREEN + f"🤖 Primary LLM: {args.provider}" +
                  (f" ({args.model})" if args.model else ""))
            if args.parser_provider and args.parser_provider.lower() != 'none':
                print(Fore.GREEN + f"🤖 Parser LLM: {args.parser_provider}" +
                      (f" ({args.parser_model})" if args.parser_model else ""))
            ...
            print(Fore.GREEN +
                  f"⏱️ Max consecutive errors: {args.max_consecutive_something_is_wrong_allowed}")
            print(Fore.GREEN +
                  f"⏱️ Max invalid reversals: {args.max_consecutive_invalid_reversals_allowed}")
            print(Fore.GREEN +
                  f"⏱️ Max no-path-found: {args.max_consecutive_no_path_found_allowed}")
            print(
                Fore.GREEN + f"⏱️ Sleep after EMPTY: {args.sleep_after_empty_step}s (skipped on NO_PATH_FOUND)")
            print(Fore.GREEN + f"🎮 GUI enabled: {not args.no_gui}")
            print(Fore.GREEN + f"🎲 Max games: {args.max_games}")
    except Exception as exception:
        print(Fore.YELLOW + f"⚠️ Warning: Could not load configuration from summary.json: {exception}")
    ...
    try:
        ...
        print(Fore.RED + f"❌ Error continuing from session: {exception}")
        traceback.print_exc()
        sys.exit(1)

...
class LLMClient:
    ...
    def __init__(self, provider: str = "yuanbao"):
    ...
    def generate_response(self, prompt: str, **kwargs) -> str:
        ...
        """
        if self.provider == "yuanbao":
            return self._generate_yuanbao_response(prompt, **kwargs)
        elif self.provider == "ollama":
            return self._generate_ollama_response(prompt, **kwargs)
        else:
            raise ValueError(f"Unsupported LLM provider: {self.provider}")
    
    def _generate_yuanbao_response(self, prompt: str, **kwargs) -> str:
        """Generate a response from Yuanbao LLM API.
        ...
        try:
            ...
            if not api_key or api_key == "your_yuanbao_api_key_here":
                print("Warning: Yuanbao API key not properly configured in .env file")
            ...
            response = requests.post(
                "https://api.yuanbao.ai/v1/chat/completions",
                headers=headers,
                json=payload
            )
            ...
            if response.status_code == 200:
                ...
                print(f"Error from Yuanbao API: {response.status_code}, {response.text}")
                return self._get_fallback_response()
            
        except Exception as e:
            print(f"Error generating response from Yuanbao: {e}")
            return self._get_fallback_response()
    ...
    def _get_fallback_response(self) -> str:
        ...
        if action == "SEARCH_VIDEOS":
            details = "uploaded today sort by upload date"

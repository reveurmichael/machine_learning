"""
Game statistics tracking and reporting functionality.
Handles step counters, time tracking, token usage, and game state analytics.
"""
...
class RoundBuffer:
    ...
    def set_apple(self, pos: Optional[list[int]]) -> None:
        """Update the current apple position.

        Accepts ``None`` to explicitly clear the position. This mirrors the
        behaviour expected by :pymeth:`core.game_rounds.RoundManager.start_new_round`.
        """
